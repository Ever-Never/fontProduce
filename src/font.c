...
{	
	fread(pfl_header->magic, 4, 1, g_prf);
	fread(&pfl_header->Size, 4, 1, g_prf);
	fread(&pfl_header->nSection, 1, 1, g_prf);
	fread(&pfl_header->YSize, 1, 1, g_prf);
	fread(&pfl_header->wCpFlag, 2, 1, g_prf);
        fread(pfl_header->reserved, 4, 1, g_prf);
